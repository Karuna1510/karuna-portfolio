...
import { allProjectsFull } from '@/data/allProjects';
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6">
          {allProjectsFull.map((project, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative z-0 flex h-full min-h-0 min-w-0 rounded-xl transition-[z-index] duration-0 hover:z-20"
            >
              <div className="flex h-full w-full min-w-0 flex-col rounded-xl border border-white/[0.08] bg-[#0e0e12] p-5 transition-all duration-300 ease-out will-change-transform group-hover:-translate-y-2 group-hover:border-primary/45 group-hover:shadow-[0_18px_44px_-12px_rgba(0,0,0,0.65),0_0_28px_-4px_hsl(195_100%_50%/0.12)] sm:p-6">
                ...
                <p className="mt-3 line-clamp-4 flex-1 text-sm leading-relaxed text-white/50">{project.description}</p>
